...
CREATE TABLE movimientos (
    ...
    CONSTRAINT fk_elementos FOREIGN KEY (nro_lia) REFERENCES elementos(nro_lia),
...
3. Crear una clase Java llamada `HelloResource` en el paquete `com.martindev.inventariolia.endpoints`.
4. Añadir el siguiente código a la clase:

```java
package com.martindev.inventariolia.endpoints;
...
Crear una clase Java llamada `ElementosResource` en el paquete `com.martindev.inventariolia.endpoints`.
...
Crear una clase Java llamada `CorsFilter` en el paquete `com.martindev.inventariolia.filters` con el siguiente código:
...
<filter>
    <filter-name>CorsFilter</filter-name>
    <filter-class>com.martindev.inventariolia.filters.CorsFilter</filter-class>
</filter>
<filter-mapping>
    <filter-name>CorsFilter</filter-name>
    <url-pattern>/*</url-pattern>
